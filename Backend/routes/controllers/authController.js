...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Query employee_onboarding table for regular users
    ...
    // Check if password matches temp_password
    const isPasswordValid = await bcrypt.compare(password, user.password_temp);
    ...
    try {
      await connection.query(
        `INSERT INTO user_system_info 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), 1)`,
        [user.id, user.email, user.name, token, deviceInfo.deviceType, deviceInfo.deviceName, 
      ...
      if (existingSession.length > 0) {
        ...
        await connection.query(
          `INSERT INTO user_concurrent_sessions 
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [user.id, user.email, 1, deviceCounts.PC, deviceCounts.Mobile, deviceCounts.Tablet, deviceCounts.Other]
        );
      }
...
exports.updatePassword = async (req, res) => {
  try {
    ...
    try {
      ...
      // Update password in employee_onboarding and reset flag
      await connection.query(
      ...
      console.log(`✅ Password updated for employee ${userId}`);
